...
Triggers are HTTP get requests, with an authentication system based off shared secrets (see the "keymanager" module), and designed to be simple enough so it can run on very small devices, like the Raspberry Pi Zero-w's, which are what generate most of the author's door, window, and motion sensor signals.
...
- - -
### keymanager: solving the digital secret bootstrapping problem
