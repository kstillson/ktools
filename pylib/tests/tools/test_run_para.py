...
def test_parallel_calcs_with_hinted_ids(tmp_path):
    ...
    argv_list = ['--plain', '--output', tempname]
    stdin_list = ['echo ^^1 + 2 | bc',
                  'echo 3 + ^^2 | bc',
                  'echo ^^3 + 4 | bc']
    assert run_para.main(argv_list, stdin_list) == 0
...
def test_mixed_stdout_and_stderr_and_mixed_return_codes(tmp_path):
    ...
    assert run_para.main(argv_list, stdin_list) == -1
...
def test_stdin_and_cmd_substitutions(tmp_path):
    tempname = str(tmp_path / "out")
    argv_list = ['--cmd', 'echo Q', '--plain', '--output', tempname, '--subst', 'Q']
    stdin_list = ['aa', 'ab', 'ac']
    assert run_para.main(argv_list, stdin_list) == 0
    with open(tempname) as f: out = f.readlines()
...
def test_timeouts(tmp_path):
    tempname = str(tmp_path / "out")
    argv_list = ['--plain', '--output', tempname, '--timeout', '1']
    stdin_list = ['sleep 0.5; echo "a"',
                  'sleep 1.5; echo "b"']
    assert run_para.main(argv_list, stdin_list) == -3
    with open(tempname) as f: out = f.readlines()
