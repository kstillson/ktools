...
def test_full_authn_cycle(web_server):
    ...
    RESP = (200, 'mysecret')
    # as client
    reg_blob = k_auth.generate_shared_secret()
    ...
    okay, status, hostname, username, sent_time = k_auth.validate_token_given_shared_secret(token, 'test-key4', reg_blob)
    assert status == 'ok'
    assert okay
    assert not username
...
def test_full_authn_cycle_with_password(web_server):
    global RESP
    RESP = (200, 'mysecret')
    reg_blob = k_auth.generate_shared_secret(user_password='pass123')

    # Let's try it with no password and confirm it fails.
    answer = kmc.query_km('key5', **KWARGS)
    path, token = INCOMING_PATH.split('?a=', 1)
    okay, status, hostname, username, sent_time = k_auth.validate_token_given_shared_secret(token, 'test-key5', reg_blob, must_be_later_than_last_check=False)
